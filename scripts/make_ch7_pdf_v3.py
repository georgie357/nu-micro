# BIO203 Ch.7 Study Guide - Version 3
# HIGHEST YIELD: Exam trap columns, Popa emphasis flags, common confusions called out.
...
OUTPUT = 'C:/Users/User/Dropbox/Nu micro/Ch 7/BIO203_Ch7_Study_Guide_v3.pdf'
...
# ── TITLE ──────────────────────────────────────────────────────────
story.append(Paragraph('BIO203 — Chapter 7 Study Guide — Version 3 (Highest Yield)', h1))
story.append(Paragraph('Microbial Biochemistry | Sources: Popa lecture slides + OpenStax Ch.7', note))
story.append(sp(1))
...
story.append(tbl([
    ...
    ['Most abundant (macronutrients)', 'H, C, O, N, P, S',
     '~99% of dry cell weight. H is single most abundant by atom count.'],
...
story.append(hr())
story.append(HY("Structural isomers: glucose, galactose, fructose all = C6H12O6 — different structure. Enantiomers discovered by Pasteur. Microbes metabolize only ONE enantiomeric form."))
story.append(tbl([
    ['Type', 'Definition', 'Example', 'Exam trap'],
    ['Structural isomers',
     'Same molecular formula; different bonding sequence',
     'Glucose, galactose, fructose all = C6H12O6',
     'They are NOT the same molecule despite identical formula. Different properties.'],
    ['Stereoisomers',
    ...
    ['Enantiomers',
     ...
     'D-aspartame = tasteless; L-aspartame = sweet.\nDiscovered by Pasteur 1848 (wine crystals).',
     'Many microbes metabolize ONLY one enantiomeric form — important for drug design and antibiotic specificity.'],
...
story.append(tbl([
    ...
    ['Triglycerides\n(triacylglycerol)',
     'Glycerol + 3 fatty acids. Simple lipid (only 2 component types).',
     'Primary energy storage. Provide >2× calories of carbs or proteins.',
     'Sebum (skin oil). Cutibacterium acnes feeds on skin lipids → acne.',
     'Simple lipid = only glycerol + fatty acids. NOT amphipathic.'],
    ['Phospholipids',
     'Glycerol + 2 fatty acids + phosphate group. Complex lipid (3+ components).',
     'AMPHIPATHIC: hydrophilic phosphate head (polar) + hydrophobic fatty acid tails (nonpolar).',
     'Form lipid bilayer of ALL cell membranes. Gram- bacteria have second outer membrane containing LPS.',
    ...
    ['Isoprenoids\n(terpenoids)',
     ...
     'Pigments (beta-carotene), fragrances (menthol, camphor, limonene), pharmaceuticals (capsaicin).',
     ''],
    ['Steroids / Sterols',
     '4 interlocking hydrocarbon rings. Sterols = steroids with -OH group.',
     'Mostly hydrophobic; sterols have some hydrophilic character',
     'Cholesterol (eukaryotes + Mycoplasma). Hopanoids (bacteria = functional equivalent). Ergosterol (fungi/protozoa = target of antifungal drugs).',
     'Bacteria do NOT have cholesterol — they have hopanoids. Ergosterol = fungal membrane = target of azole antifungals.'],
    ['Waxes\n(incl. mycolic acid)',
     'Long-chain isoprenoids. Water-resistant, hard at room temp.',
     'Waterproofing',
     'Mycolic acid in Mycobacterium tuberculosis cell wall → waxy → acid-fast staining. Sebum = wax esters + triacylglycerol in skin.',
     'Mycolic acid = waxy lipid = ACID-FAST = Mycobacterium. This connection is always tested.'],
], [0.95*inch, 1.5*inch, 1.3*inch, 1.8*inch, 1.65*inch]))
story.append(sp(1))
...
story.append(tbl([
    ...
    ['2° Secondary',
     ...
     'α-helix: coiled spring, H-bond every 4 AA.\nβ-sheet: pleated structure from H-bonds between adjacent segments.\nKey: H-bonds between backbone atoms, not R groups.'],
    ['3° Tertiary',
...
story.append(HY("MALDI-TOF is now standard in clinical labs. Lancefield groups = serological grouping of Streptococcus. FAME = fatty acid methyl ester profiling. Know what each method analyzes."))
story.append(sp(1))
...
story.append(tbl([
    ...
    ['MALDI-TOF\nMass Spectrometry',
     ...
     'NOW STANDARD in clinical labs. Fast, accurate. Identifies organisms that traditional methods cannot. Popa emphasized this heavily.'],
    ['FAME analysis',
    ...
    ['PLFA analysis',
     ...
     'Same as FAME but specifically from phospholipid fraction (saponification first)',
     'Similar to FAME but more specific. Used for environmental samples.'],
    ['Serological tests\n(Lancefield)',
     'Carbohydrates on cell surface glycoproteins',
     'Antibodies bind specific surface carbohydrate antigens → agglutination (clumping)',
     'Lancefield grouping: identifies Streptococcus species by surface carbohydrate type (Group A = S. pyogenes, Group B = S. agalactiae). Rapid strep test uses this principle.'],
    ['Proteomic analysis',
    ...
    ['PHB granules\n(storage)',
     'Poly-beta-hydroxybutyrate (carbon/energy storage)',
     'Presence or absence distinguishes species',
     'P. aeruginosa (human pathogen) does NOT accumulate PHB — useful differentiating test.'],
...
story.append(tbl([
    ...
    ['Hopanoids vs cholesterol vs ergosterol',
     'Cholesterol = eukaryotes + Mycoplasma (wall-less bacteria).\nHopanoids = bacteria (functional equivalent of cholesterol, NOT cholesterol itself).\nErgosterol = fungi and some protozoa. TARGET of antifungal drugs (e.g., fluconazole, amphotericin B).'],
    ['Mycolic acid',
...
story.append(tbl([
    ...
    ['"Bacteria have cholesterol in their membranes"', 'Bacteria have HOPANOIDS (not cholesterol). Mycoplasma (wall-less) may have cholesterol.'],
    ...
    ['"All bacteria have the same membrane sterol"', 'Most bacteria have hopanoids; Mycoplasma has cholesterol; fungi have ergosterol'],
], [2.8*inch, 4.2*inch]))
