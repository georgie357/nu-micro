# BIO203 Ch.1 & Ch.2 - Multiple Choice Questions - Version 4
# V4: Lecture-slide-only. REMOVED: Hooke (Q7), Hooke vs Leeuwenhoek (Q8),
#     Janssen (Q9), spontaneous generation (Q10), al-Razi (Q35)
#     ADDED: Bergey's Manual Q31
# 51 questions total. Ch1 = Q1-31, Ch2 = Q32-51.
...
OUTPUT = r"C:\Users\User\Dropbox\Nu micro\chapter 1 and 2\BIO203_Ch1_Ch2_MC_Questions_v4.pdf"
...
story += [
    ...
    sp(4),
    Paragraph("Chapters 1 &amp; 2 — 51 Multiple Choice Questions — Version 4 (Lecture Slides Only)",
              ParagraphStyle('TS', fontName='Helvetica-Bold', fontSize=13, alignment=TA_CENTER)),
    sp(3),
    Paragraph("Source: Popa lecture slides only | Answer key included | Textbook-only topics excluded",
...
questions = [
    ...
    (8,
     "Which of the following is NOT one of Pasteur's contributions as taught in lecture?",
     ["A. Proved microbes cause fermentation",
      "B. First experimental proof that ONE specific microbe causes ONE specific disease",
      "C. Invented pasteurization",
      "D. Developed the rabies vaccine"],
     "B", "Popa slide 8 — B is Koch's contribution (anthrax 1876)"),
    ...
    (31,
     "Bergey's Manual of Systematic Bacteriology classifies bacteria based on:",
     ["A. Colony morphology and color on solid media only",
      "B. Geographic location where the organism was first isolated",
      "C. Genetic, biochemical, and immunological characteristics",
      "D. Size and shape as observed by light microscopy alone"],
     "C", "Popa slide 22"),
    ...
    (36,
     ...
     "A", "Popa slides 16–18 — Human eye ~200 µm; LM ~200 nm; TEM ~2.5 nm"),
    ...
    (51,
     ...
     ["A. Phase-contrast", "B. Darkfield",
      "C. Confocal laser scanning", "D. Fluorescence"],
     "C", "Popa slide 14"),
]
...
for (num, qtext, opts, correct, citation) in questions:
    if num == 1 and not ch1_done:
        ...
        story.append(sp(4))
        ch1_done = True
    if num == 32 and not ch2_done:
        story.append(Paragraph("CHAPTER 2 QUESTIONS (Q32–Q51)", h2))
        story.append(HRFlowable(width=W, thickness=0.75, color=colors.HexColor('#1a1a6e')))
        story.append(sp(4))
        ch2_done = True
...
story.append(Paragraph("ANSWER KEY — Version 4", h1))
...
col_w = [0.5*inch, 0.4*inch] * 6
story.append(tbl(rows, col_w))
story.append(sp(8))
story.append(Paragraph(
    "Sources: Popa BIO203 lecture slides (Ch.1: 29 slides; Ch.2: 33 slides). "
    "Version 4 — 51 questions, lecture-slide content only. Textbook-only topics excluded.",
